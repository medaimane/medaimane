## 👨‍💻 Expo React Native Apps / Senior Software Engineer

### Top 3% of Talented Freelancers on @Upwork 🌟

_I help Founders and Startups build Mobile Apps & SaaS/AaaS Products with @ReactNative & @Expo,  @Reactjs & @Nextjs, @Supabase/@Firebase_

#### TL;DR:

- 🌟 Top Rated Plus with a 100% Job Success on Upwork.
- 🚀 8+ years specializing in Expo & React/React Native ecosystem.
- 💡 Proven track record with 5-star reviews from top clients.

#### What I Bring:

- 🏆 Elevate your team as a Tech Lead or Senior Engineer.
- 🌐 Global collaboration experience in Healthcare, Travel, Art, Entertainment, and Education.
- 🛠 Core Tech Stack: Expo, React Native (including React/NextJS) with TypeScript.
- 🚀 Expert in integrating AI, Cloud solutions, and third-party platforms.
- 🧪 Follow advanced development principles: Design Patterns, TDD, Clean Code, Clean Architecture.
- 🔄 Experienced in project management and agile methodologies (Kanban & Scrum).
- 🤝 Strong sense of product ownership, team leading, and technical decision-making.
- 🛤️ Proficient in MVP launch and product development roadmapping.
- 💡 Creative and critical thinking with excellent communication skills.

#### Let's Build Something Amazing:

- 📱 Arcitect complex mobile & frontend apps.
- 🚀 Crafted, pixel-perfect UIs for exceptional user experiences.
- 🌐 Worked with OpenAI, AWS, Google Services, Clerk, Firebase, Supabase, and more.
- 🛠 Set up CI/CD pipelines for seamless deployment (Expo EAS/Bitrise and more).
- 🧪 Committed to top-notch quality: Modular, scalable, clean, and well-tested code.

### Thinking of a project?🔥

Let's connect & bring your vision to reality!

<a href="https://www.upwork.com/freelancers/medaimane" target="_blank">Hire Me On Upwork</a> | <a href="https://lnk.bio/medaimane" target="_blank">More Links</a>

### Support Me:

<a href="https://www.buymeacoffee.com/medaimane" target="_blank">
  <img src="https://cdn.buymeacoffee.com/buttons/v2/default-yellow.png" width="130" />
</a>

_Thanks for visiting my profile!_

<!--

---

### Github stats

![Mohamed's github stats](https://github-readme-stats.vercel.app/api?username=medaimane&count_private=true&show_icons=true&theme=radical)
-->
